...
<form action="${pageContext.request.contextPath}/member/updateEstateEnd.do" method="post" onsubmit="return validate();"
enctype="multipart/form-data">
	<input type="hidden" name="estateNo" value="${estate.getEstateNo()}" />
	<input type="hidden" name="memberNo" value="${estate.getMemberNo()}" />
	<table>
		<tr style="display:none;">
			<th>일반회원번호</th>
			<td><input type="text" name="MemberNo" id="MemberNo" value="${estate.getMemberNo() }" /></td>
		</tr>
		<tr style="display:none;">
			<th>중개인회원번호</th>
		...
		<c:if test="${estate.getAddressDetail() != null }">
			<c:if test="${estate.getEstateType() eq 'A'.charAt(0) }">
				<tr>
					<th>상세정보</th>																			
					<td>
					<input type="number" name="address2" id="address2" class="addr" value="${estate.getAddressDetail().substring(0, estate.getAddressDetail().indexOf('동'))}" required>동
			...
			<c:if test="${estate.getEstateType() ne 'A'.charAt(0) }">
				<tr>
					<th>상세정보</th>																			
					<td>
					<input type="hidden" name="address2" id="address2" class="addr" value="0">
					<input type="number" name="address3" id="address3" class="addr" value="${estate.getAddressDetail().substring(0, estate.getAddressDetail().indexOf('층'))}" required>층
					</td>
				</tr>
			</c:if>	
		</c:if>
